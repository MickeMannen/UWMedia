...
class TestRelease:
    ...
    def test_metadata_photo(self):
        photo_file = TEST_DATA_DIR / "DSC03861.JPG"
        handler = MetadataHandler()
        ...
        assert creation_date.month == 1
        assert creation_date.day == 30
        assert creation_date.hour == 11
        assert creation_date.minute == 41
        assert creation_date.second == 32

    # 2. Garmin Parser Tests
    def test_garmin_parser(self):
        parser = GarminParser()
    ...
    # 4. Color Correction
    def test_color_correction_video(self):
    ...
    def test_overlay_video(self):
        ...
        cmd = [
            ...
            "--filename-format", "%Y%m%d_%H%M%S_test05_overlay_generic"
        ]
        subprocess.run(cmd, check=True)
        
        target = list(OUTPUT_DIR.glob("*_test05_overlay_generic.mp4"))[0]
    ...
    def test_overlay_photo(self):
        src = TEST_DATA_DIR / "DSC03866.JPG"
        layout = COMPUTERS_DIR / "generic_depth_temp.zip"
        logs = FIT_DIR
        cmd = [
            "python3", "cli_main.py", str(src), str(OUTPUT_DIR),
