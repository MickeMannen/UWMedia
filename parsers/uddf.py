    ...
    def parse(self, file_path: Path) -> List[Dive]:
        ...
        for dive_elem in root.xpath("//*[local-name()='dive']"):
            ...
            for sample in dive_elem.xpath(".//*[local-name()='waypoint']"):
                ...
                if po2_str:
                    try:
                        last_po2 = round(float(po2_str), 2)
                    except (ValueError, TypeError):
                        pass
                po2 = last_po2
            ...
            if waypoints:
                ...
                try:
                    # Convert internal gas_mixes to GasDefinition list
                    deco_gases = []
                    for mix_id, mix in gas_mixes.items():
                        # MOD calculation (PPO2 1.4 for OC)
                        mod = ((1.4 / (mix["o2"] / 100.0)) - 1.0) * 10.0
